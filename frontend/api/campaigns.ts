...
function campaign(row: any) {
  return {
    ...
    duration: String(row.duration_days),
    imageUrl: row.image_url,
    thumbnailUrl: row.thumbnail_url ? [row.thumbnail_url] : [],
    ...
    status: row.status,
    totalRaised: String(row.total_raised),
    donationCount: String(row.donation_count),
    websiteUrl: row.website_url ? [row.website_url] : [],
    twitterUrl: row.twitter_url ? [row.twitter_url] : [],
    telegramUrl: row.telegram_url ? [row.telegram_url] : [],
...
export default async function handler(req: any, res: any) {
  try {
    if (req.method === 'GET') {
      ...
      const rows = id
        ? await sql`select * from campaigns where id = ${id} limit 1`
        : creator
          ? await sql`select * from campaigns where creator_wallet_address = ${creator} order by created_at desc`
    ...
    const rows = await sql`
      insert into campaigns (id, title, description, goal, duration_days, image_url, thumbnail_url, creator_wallet_address, end_at, website_url, twitter_url, telegram_url)
      values (${id}, ${title}, ${description}, ${goal.toString()}, ${duration}, ${String(body.imageUrl || '')}, ${body.thumbnailUrl || null}, ${walletAddress}, ${endAt}, ${body.websiteUrl || null}, ${body.twitterUrl || null}, ${body.telegramUrl || null})
      returning *
    `;
