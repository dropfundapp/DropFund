...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'GET') {
      const rows = await sql`select solana_address, display_name, image_url from users where privy_did = ${privyDid} limit 1`;
      const user = rows[0];
      return json(res, {
    ...
    const updated = await sql`
      with prior as (
        ...
        returning prior.display_name as old_name, user_record.display_name as new_name, user_record.solana_address
      ), updated_campaigns as (
        update campaigns
        set creator_display_name = changed.new_name
        from changed
        where campaigns.creator_wallet_address = changed.solana_address
      ...
      select new_name as name from changed
    `;
    if (!updated[0]) return json(res, { error: 'Profile was not found' }, 404);
    return json(res, { ok: true, name: updated[0].name });
  } catch (error: any) {
