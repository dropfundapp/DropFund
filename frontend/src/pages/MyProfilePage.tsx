...
import { BarChart3, Copy, DollarSign, Plus, UserRound, Camera, X } from 'lucide-react';
...
export default function MyProfilePage() {
  ...
  return (
      <div className="min-h-screen bg-[#131313] pt-16 pb-8 text-white">
          ...
          <section className="grid gap-10 py-8 lg:grid-cols-[minmax(0,1fr)_minmax(420px,0.7fr)]">
            <div>
              ...
              <div className="mt-6 flex w-full items-center justify-between rounded-xl bg-[#1d1e1f] p-4">
                <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#282b30]"><DollarSign className="h-5 w-5" /></span><div><p className="text-xs text-white/45">Total cash</p><p className="font-semibold">{balanceLoading ? 'Loading...' : usdcBalance !== null ? `${formatUsdc(usdcBalance)} USDC` : 'Unavailable'}</p>{balanceError ? <p className="max-w-[220px] break-words text-[11px] text-rose-400">{balanceError}</p> : null}</div></div>
        ...
        {editOpen && <div className="fixed inset-0 z-[10002] flex items-center justify-center p-4 backdrop-blur-[3px]" onClick={() => setEditOpen(false)}><div className="w-full max-w-md rounded-2xl bg-[#1d1e1f] p-6 text-white" onClick={(event) => event.stopPropagation()}><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-semibold">Edit profile</h2><Button variant="ghost" size="icon" onClick={() => setEditOpen(false)}><X className="h-5 w-5" /></Button></div><label className="mb-2 block text-sm text-white/60">Display name</label><input value={profileName} onChange={(event) => setProfileName(event.target.value)} className="mb-5 w-full rounded-xl border border-[#282b30] bg-[#282b30] px-4 py-3 text-white outline-none" maxLength={32} /><label className="mb-2 block text-sm text-white/60">Profile picture</label><label className="mb-6 flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-[#282b30] bg-[#282b30] p-4"><Camera className="h-5 w-5" /><span>{profileImage ? 'Change picture' : 'Upload picture'}</span><input type="file" accept="image/*" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => setProfileImage(String(reader.result)); reader.readAsDataURL(file); }} /></label><Button className="h-[3.2rem] w-full text-lg font-semibold" onClick={saveProfile}>Save profile</Button></div></div>}
